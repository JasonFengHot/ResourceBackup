...
if ((-e $new_alps) && (-e $new_git)){
	open(my $log_out , "<" , $file1) or die ("Could not open file .project.log!!!");#不要删除.project.log,不然会报错,如果不小心删除,需要将alps,sagereal和.git先放回原来的目录
...
if ((-e "$ARGV[0]/alps") && (-e "$ARGV[0]/.git")){
	...
	open(my $log_in, ">", $file1) or die ("Could not open file .project.log!!!");
	print $log_in $ARGV[0];
	close $log_in;
